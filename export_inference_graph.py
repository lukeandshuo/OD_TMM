...
flags.DEFINE_string('input_type', 'image_tensor', 'Type of input node. Can be '
                    ...
                    '`tf_example`]')
flags.DEFINE_string('pipeline_config_path', '/data/SensiacDay/TF_Record/faster_rcnn_resnet101_sensiac.config',
                    'Path to a pipeline_pb2.TrainEvalPipelineConfig config '
                    'file.')
flags.DEFINE_string('checkpoint_path', 'experiments/trained_model/IR/train/model.ckpt-13470', 'Optional path to checkpoint file. '
                    'If provided, bakes the weights from the checkpoint into '
                    'the graph.')
flags.DEFINE_string('inference_graph_path', 'frozen_models/frozen_IR_inference_graph.pb', 'Path to write the output '
                    'inference graph.')
